...
class Joker:
    def response(self, flow: mitmproxy.http.HTTPFlow):
        if "daydata.php?city=" in flow.request.url:
            # 替换请求链接
            print(flow.request.url)
            list1=[" document.write('检测到非法调试, 请关闭调试终端后刷新本页面重试!')",
              'document.write("<br/>")',
              'document.write("Welcome for People, Not Welcome for Machine!")',
              "debugflag = true"]

            for i in list1:
                flow.response.text=flow.response.text.replace(i,'debugflag = false')
...
#下面这段代码的作用是https请求的时候证书错误但是用了之后无法有效加载修改数据，不知道为什么
